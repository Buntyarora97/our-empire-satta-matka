...
export default function Deposits() {
  ...
  const { data, isLoading } = useAdminGetDeposits(params, { query: { queryKey: getAdminGetDepositsQueryKey(params) } });
  const approve = useAdminApproveDeposit();
  const reject = useAdminRejectDeposit();

  const handleApprove = (id: string) => {
    approve.mutate({ id }, {
      onSuccess: () => { toast({ title: "Deposit approved" }); qc.invalidateQueries({ queryKey: getAdminGetDepositsQueryKey(params) }); },
    });
  };

  const handleReject = () => {
    reject.mutate({ id: rejectId, data: { reason: rejectReason } }, {
      onSuccess: () => { toast({ title: "Deposit rejected" }); qc.invalidateQueries({ queryKey: getAdminGetDepositsQueryKey(params) }); setRejectOpen(false); setRejectReason(""); },
    });
  };
    ...
    <AdminLayout>
        ...
        <Tabs value={activeTab} onValueChange={v => { setActiveTab(v); setPage(1); }}>
          <TabsList className="bg-[#1a1a1a] border border-[#2a2a2a]">
            {["pending", "completed", "rejected"].map(t => (
              <TabsTrigger key={t} value={t} className="capitalize data-[state=active]:bg-[#48723e] data-[state=active]:text-white" data-testid={`tab-deposits-${t}`}>{t}</TabsTrigger>
            ))}
          </TabsList>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-[#2a2a2a]">
                            {["User", "Amount", "UTR Number", "Status", "Date", "Actions"].map(h => (
                        ...
                        <tbody>
                          {(data?.transactions || []).map((t: any) => (
                            <tr key={t.id} className="border-b border-[#2a2a2a]/50 hover:bg-[#111111]" data-testid={`row-deposit-${t.id}`}>
                              ...
                              <td className="py-3 px-4 text-[#a0a0a0] font-mono text-xs">{t.utrNumber || "-"}</td>
                              <td className="py-3 px-4">
                              ...
                              <td className="py-3 px-4">
                                {t.status === "pending" && (
                                  <div className="flex gap-1">
                                    <Button size="sm" variant="ghost" className="h-7 px-2 text-green-400 hover:bg-green-400/10" onClick={() => handleApprove(t.id)} data-testid={`button-approve-deposit-${t.id}`}>
                                      <CheckCircle className="h-3.5 w-3.5" />
                                    </Button>
                                    <Button size="sm" variant="ghost" className="h-7 px-2 text-red-400 hover:bg-red-400/10" onClick={() => { setRejectId(t.id); setRejectOpen(true); }} data-testid={`button-reject-deposit-${t.id}`}>
                                      <XCircle className="h-3.5 w-3.5" />
                                    </Button>
                                  </div>
                                )}
                                {t.adminNote && <div className="text-xs text-[#a0a0a0] mt-1">{t.adminNote}</div>}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      {(data?.transactions?.length ?? 0) === 0 && <div className="text-center py-8 text-[#a0a0a0]">No {tab} deposits</div>}
                    </div>
                  )}
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="bg-[#1a1a1a] border-[#2a2a2a] text-white">
          ...
          <Textarea placeholder="Reason for rejection..." value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="bg-[#111111] border-[#2a2a2a] text-white" data-testid="input-reject-reason" />
